...
static unsigned emit_power_as_shift(ivl_scope_t scope, ivl_expr_t expr,
		unsigned wid) {
	error("check: emit_power_as_shift");
	int rtype;
	int64_t value, scale;
	...
	if (scale != 1) {
		if (is_signed_rval) {
			fprintf(g_out, "(%" PRId64, scale);
		} else {
			fprintf(g_out, "(%u'h%" PRIx64,
					ivl_expr_width(rval), scale);
		}
		fprintf(g_out, " * ");
	}
	emit_expr(scope, rval, 0, 0, 0, 0);
	if (scale != 1) fprintf(g_out, ")");
	if (is_signed_rval) fprintf(g_out, ")");
	return 1;
}
...
/*static void emit_class_property(ivl_scope_t scope, ivl_expr_t expr, unsigned wid) {
	...
	emit_scope_call_path(scope, ivl_signal_scope(sig));
	emit_id(ivl_signal_basename(sig));
	fprintf(g_out, ".%s", ivl_expr_name(expr));
}*/
...
/*static void emit_expr_ips(ivl_scope_t scope, ivl_expr_t sig_expr,
	...
	if ((!g_allow_signed) && (sel_type == IVL_SEL_IDX_DOWN) &&
...
static SMTUnary* emit_expr_unary(ivl_scope_t scope, ivl_expr_t expr, unsigned wid,
	...
	switch (ivl_expr_opcode(expr)) {
		...
		case 'r':
			// A cast is a noop.
			emit_expr(scope, oper1, 0, 0, 0, 0, smt_unary);
			break;
		case 'I':
			fprintf(g_out, "(++");
			emit_expr(scope, oper1, wid, 0, 0, is_full_prec, smt_unary);
			fprintf(g_out, ")");
