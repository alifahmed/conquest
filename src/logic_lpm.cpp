...
static unsigned emit_drive(ivl_drive_t drive) {
	switch (drive) {
		...
		case IVL_DR_PULL:
			...
			break;
		case IVL_DR_STRONG:
			fprintf(g_out, "strong");
			break;
		case IVL_DR_SUPPLY:
			fprintf(g_out, "supply");
			break;
		default:
			return 1;
			break;
...
static void emit_posedge_dff_prim(void) {
	fprintf(g_out, "\n");
	fprintf(g_out, "/* Icarus generated UDP to represent a synthesized "
			"positive edge D-FF. */\n");
	fprintf(g_out, "primitive IVL_posedge_DFF "
			"(q, clk, en, d, clr, set);\n");
	fprintf(g_out, "%*coutput q;\n", g_ind_incr, ' ');
	fprintf(g_out, "%*cinput clk, en, d, clr, set;\n", g_ind_incr, ' ');
	fprintf(g_out, "%*creg q;\n", g_ind_incr, ' ');
	fprintf(g_out, "%*ctable\n", g_ind_incr, ' ');
	fprintf(g_out, "%*cr 1 0 0 0 : ? : 0 ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*cr 1 1 0 0 : ? : 1 ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*cp 1 0 0 0 : 0 : - ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*cp 1 1 0 0 : 1 : - ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*cp x 0 0 0 : 0 : - ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*cp x 1 0 0 : 1 : - ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*cn ? ? 0 0 : ? : - ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*c* 0 ? 0 0 : ? : - ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*c? * ? ? ? : ? : - ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*c? ? * ? ? : ? : - ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*c? ? ? * ? : ? : - ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*c? ? ? ? * : ? : - ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*c? ? ? 0 1 : ? : 1 ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*c? ? ? 0 x : 1 : - ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*c? ? ? 0 x : 0 : x ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*c? ? ? 1 ? : ? : 0 ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*c? ? ? x 0 : 0 : - ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*c? ? ? x 0 : 1 : x ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*c? ? ? x x : ? : x ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*c? ? ? x 1 : ? : x ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*cendtable\n", g_ind_incr, ' ');
	fprintf(g_out, "endprimitive\n");
}
...
static void emit_negedge_dff_prim(void) {
	...
	fprintf(g_out, "/* Icarus generated UDP to represent a synthesized "
			"negative edge D-FF. */\n");
	fprintf(g_out, "primitive IVL_negedge_DFF "
			"(q, clk, en, d, clr, set);\n");
	fprintf(g_out, "%*coutput q;\n", g_ind_incr, ' ');
	fprintf(g_out, "%*cinput clk, en, d, clr, set;\n", g_ind_incr, ' ');
	fprintf(g_out, "%*creg q;\n", g_ind_incr, ' ');
	fprintf(g_out, "%*ctable\n", g_ind_incr, ' ');
	fprintf(g_out, "%*cf 1 0 0 0 : ? : 0 ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*cf 1 1 0 0 : ? : 1 ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*cn 1 0 0 0 : 0 : - ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*cn 1 1 0 0 : 1 : - ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*cn x 0 0 0 : 0 : - ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*cn x 1 0 0 : 1 : - ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*cp ? ? 0 0 : ? : - ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*c* 0 ? 0 0 : ? : - ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*c? * ? ? ? : ? : - ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*c? ? * ? ? : ? : - ;\n", 2 * g_ind_incr, ' ');
	fprintf(g_out, "%*c? ? ? * ? : ? : - ;\n", 2 * g_ind_incr, ' ');
...
static void emit_lpm_ff(ivl_scope_t scope, ivl_lpm_t lpm) {
	...
	if (negedge) {
		fprintf(g_out, "IVL_negedge_DFF");
	} else {
		fprintf(g_out, "IVL_posedge_DFF");
	}
	...
	if (ivl_lpm_width(lpm) > 1) {
		fprintf(g_out, " synth_%p [%u:0]", lpm, ivl_lpm_width(lpm) - 1U);
	}
	fprintf(g_out, " (");
	/* Emit the q pin. */
	...
	if (nex) {
		emit_nexus_as_ca(scope, nex, 0, 0);
		if (have_data | have_sset) fprintf(g_out, " & ");
		if (have_data & have_sset) fprintf(g_out, "(");
	}
	nex = ivl_lpm_sync_set(lpm);
	if (nex) {
...
void emit_lpm(ivl_scope_t scope, ivl_lpm_t lpm) {
    ...
    fprintf(g_out, " = ");
	smt_assign->rval = emit_lpm_as_ca(scope, lpm, 0);
	fprintf(g_out, ";");
	single_indent = 1;
...
void emit_tran(ivl_scope_t scope, ivl_switch_t tran) {
	...
	switch (ivl_switch_type(tran)) {
		...
		case IVL_SW_RTRANIF0:
			fprintf(g_out, "rtranif0");
			dly_count = 3;
			pins = 3;
			break;
		case IVL_SW_RTRANIF1:
			fprintf(g_out, "rtranif1");
			dly_count = 3;
			pins = 3;
			break;
		case IVL_SW_TRAN:
			...
			break;
		case IVL_SW_TRANIF0:
			fprintf(g_out, "tranif0");
			dly_count = 3;
			pins = 3;
			break;
		case IVL_SW_TRANIF1:
			fprintf(g_out, "tranif1");
			dly_count = 3;
			pins = 3;
			break;
		case IVL_SW_TRAN_VP:
