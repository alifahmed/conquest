...
void emit_scaled_delay(ivl_scope_t scope, uint64_t delay) {
	int scale = ivl_scope_time_units(scope) - g_precision;
	...
	if (real_dly) {
		fprintf(g_out, "%" PRIu64".%s", delay, frac);
	} else {
		if (delay & 0xffffffff80000000) {
			fprintf(g_out, "(64'd%" PRIu64")", delay);
		} else {
			fprintf(g_out, "%" PRIu64, delay);
		}
	}
	free(frac);
}
...
void emit_scaled_delayx(ivl_scope_t scope, ivl_expr_t expr, unsigned is_stmt) {
	...
	if (type == IVL_EX_DELAY) {
		...
		int exponent = ivl_scope_time_units(scope) - g_precision;
		assert(exponent >= 0);
