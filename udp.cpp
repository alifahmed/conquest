...
static void emit_udp(ivl_udp_t udp)
{
      unsigned idx, count;
      fprintf(g_out, "\n");
      fprintf(g_out, "/* This primitive was originally defined in "
                        "file %s at line %u. */\n",
                        ivl_udp_file(udp), ivl_udp_lineno(udp));
      fprintf(g_out, "primitive ");
      ...
      fprintf(g_out, ");\n");
      fprintf(g_out, "%*coutput ", g_ind_incr, ' ');
      emit_id(ivl_udp_port(udp, 0));
      fprintf(g_out, ";\n");
      for (idx = 1; idx <= count; idx += 1) {
	    fprintf(g_out, "%*cinput ", g_ind_incr, ' ');
	    emit_id(ivl_udp_port(udp, idx));
	    fprintf(g_out, ";\n");
      }
      if (ivl_udp_sequ(udp)) {
	    ...
	    fprintf(g_out, "%*cinitial ", g_ind_incr, ' ');
	    emit_id(ivl_udp_port(udp, 0));
	    fprintf(g_out, " = 1'b%c;\n", init);
      }
      fprintf(g_out, "\n");
